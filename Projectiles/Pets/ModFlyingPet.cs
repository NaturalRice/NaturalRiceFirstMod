...
namespace NaturalRiceFirstMod.Projectiles.Pets
{
    /// <summary>
    ...
    public abstract class ModFlyingPet : ModPet
    {
        ...
        /// Default is 60f
        /// </summary>
        public override float FlyingInertia => 60f;//原先60f

        public sealed override void PetBehaviour()
        {
            //Projectile.rotation = 0f;//或者干脆不转了，转你妈！
            ...
            {
                flyingSpeed *= 1.25f;//1.25f
                flyingInertia *= 0.75f;//0.75f
            }
            ...
            switch (state)
            {
                case States.Flying:
                    ...
                    /*if (ShouldFlyRotate)
                    {
                        Projectile.rotation = Projectile.velocity.X * 0.01f;//这里我将飞行宠物远离玩家时的倾斜角从0.1f调小了一些，别太荒谬
